...
def process_message(event):
    ...
    if event['messageType'] == 'CodeScanReport':
        ...
        if event['reportType'] == 'OWASP-Dependency-Check':
            ...
            for i in range(dep_pkgs):
                if "packages" in event['report']['dependencies'][i]:
                    ...
                    finding_description = f"Package: {event['report']['dependencies'][i]['packages'][0]['id']}, Confidence: {confidence}, URL: {url}" # pylint: disable=C0301
                    ...
                    securityhub.import_finding_to_sh(i, account_id, region, created_at,
                                                source_repository, source_branch, source_commitid,
                                                build_id, report_url, finding_id, generator_id,
                                                normalized_severity, severity, finding_type,
                                                FINDING_TITLE,
                                                finding_description,
                                                BEST_PRACTICES_OWASP)
        ...
        if event['reportType'] == 'PHPStan':
            ...
            for i in range(report_count):
                for filename in event['report']['files']:
                    finding_id = f"{i}-{report_type.lower()}-{build_id}"
                    finding_description = f"Message: {event['report']['files'][filename]['messages'][0]['message']}, file: {filename}, line: {event['report']['files'][filename]['messages'][0]['line']}" # pylint: disable=C0301
                    created_at = datetime.now(timezone.utc).isoformat()
                    normalized_severity = 60
                    ### find the vulnerability severity level
                    is_ignorable = f"{event['report']['files'][filename]['messages'][0]['ignorable']}" # pylint: disable=C0301
                    ...
                    securityhub.import_finding_to_sh(i, account_id, region, created_at,
                                                ...
                                                normalized_severity, severity, finding_type,
                                                FINDING_TITLE,
                                                finding_description, BEST_PRACTICES_OWASP)
        ### SonarQube SAST scanning report parsing
            ...
            for i in range(report_count):
                finding_id = f"{i}-{report_type.lower()}-{source_repository}-{source_branch}-{build_id}" # pylint: disable=C0301
                finding_description = f"{event['report']['issues'][i]['type']}-{event['report']['issues'][i]['message']}-{i}, component: {event['report']['issues'][i]['component']}" # pylint: disable=C0301
                created_at = datetime.now(timezone.utc).isoformat()
        ...
        ### OWASP Zap SAST scanning report parsing
            ...
            for alertno in range(alert_count):
                ...
                if riskletters == 'Hig':
                    ...
                    normalized_severity = 60
                elif riskletters == 'Low' or riskletters == 'Info':
                    normalized_severity = 30
                else:
                    normalized_severity = 90
                instances = len(event['report']['site'][0]['alerts'][alertno]['instances'])
                finding_description = f"{alertno}-Vulerability:{event['report']['site'][0]['alerts'][alertno]['alert']}-Total occurances of this issue:{instances}" # pylint: disable=C0301
                finding_id = f"{alertno}-{report_type.lower()}-{build_id}"
                created_at = datetime.now(timezone.utc).isoformat()
                ### Calling Securityhub function to post the findings
                securityhub.import_finding_to_sh(alertno, account_id, region, created_at,
                                            source_repository, source_branch, source_commitid,
                                            build_id, report_url, finding_id, generator_id,
                                            normalized_severity,
